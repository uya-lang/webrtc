#!/usr/bin/env python3
"""Validate SRTP bench baseline rows and zero-allocation assertions."""
...
def validate_rows(rows: list[dict], source: str) -> None:
    ...
    for name in sorted(EXPECTED_ROWS):
        ...
        for key in ["throughput_mb_s", "packets_per_s", "p95_ns", "p99_ns"]:
            if key not in row:
                raise AssertionError(f"{source}:{name}: missing {key}")
...
def main() -> None:
    ...
    validate_rows(rows, "bench_srtp.jsonl")

    with tempfile.NamedTemporaryFile(prefix="bench_srtp_", suffix=".jsonl", delete=False) as tmp:
