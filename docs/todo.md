...
- [ ] 定义 `IceCandidate`、`CandidatePair`、`IceAgent`。
...
- [ ] 先增加 DTLS record parser 截断、epoch/sequence、fragment/reassembly failing tests。
...
- [ ] 先增加 RTP/RTCP parser fixtures、截断包、extension 边界 failing tests。
- [ ] 先建立 RTP/RTCP parser microbench 和 allocation count 基线。
...
- [ ] 实现 Opus RTP packetize/depacketize。
...
- [ ] 先增加虚拟网络 benchmark：带宽下降/恢复、queue delay、loss、jitter。
