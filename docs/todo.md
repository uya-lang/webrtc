...
- [x] 打通 Uya 进程直连 Chrome 的 FFmpeg codec 音视频通话：FFmpeg raw/encoded frame -> WebRTC RTP payload packetizer -> SRTP -> UDP -> Chrome inbound RTP，且不经浏览器内部 `pc1`/`pc2` loopback。
  ...
  - [x] 支持手工预览指定 MP4 源：`make preview-ffmpeg-chrome-call MP4=/absolute/path/to/source.mp4` 会用 FFmpeg probe 按输入 MP4 实际尺寸和完整 duration 转成 raw I420 + mono s16le，Uya sender 通过 `--raw-video-i420` / `--raw-audio-s16le` / `--video-width` / `--video-height` / `--media-duration-us` 连续读取并把整段 MP4 推给 Chrome。
    - verified 2026-06-04: `make test-ffmpeg-chrome-call` 生成 1s 1920x1080 MP4 并覆盖 `--source-mp4`，MP4 manual preview `source_kind=mp4`、`preview_size=1920x1080`、`preview_duration_us=1000000`、`chrome_video_size=1920x1080`、`chrome_audio_packets=29`、`chrome_video_packets=424`、`chrome_video_frames=29`、`sender_ffmpeg_frames=58`、`sender_rtp_packets=453`、`sender_srtp_packets=453`、`sender_srtcp_packets=2`、`sender_rtcp_receiver_reports=1`、`sender_udp_packets=455`。
    - note: 当前原始尺寸 MP4 全量推送不设置固定宽高或固定 6s 时长上限，要求偶数宽高且源 MP4 有可 probe 的 duration；实际可推尺寸/时长受本机 raw 文件生成、磁盘、内存、VP8 编码速度和 Chrome 解码能力限制。
  - [x] 实现 VP8 RTP fragmentation 和按输入尺寸分配的 encoded frame buffer，支持不降分辨率的 MP4 源直推 Chrome。
    - verified 2026-06-04: `bash tests/check_phase21_ffmpeg_direct_sender.sh` 覆盖 VP8 fragment packetize/reassembly；`make test-ffmpeg-chrome-call` 的 1s 1080p MP4 段 `sender_rtp_packets=453`、`sender_srtp_packets=453`、`sender_udp_packets=455`。
