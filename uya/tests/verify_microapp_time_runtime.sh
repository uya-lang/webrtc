...
SOURCE="$ROOT_DIR/tests/fixtures/microapp/test_std_microapp_time_runtime.uya"
RUN_LOG="$(mktemp /tmp/verify_microapp_time_run.XXXXXX.log)"
LOADER_LOG="$(mktemp /tmp/verify_microapp_time_loader.XXXXXX.log)"
BUILD_LOG="$(mktemp /tmp/verify_microapp_time_build.XXXXXX.log)"
UAPP_PATH="$(mktemp /tmp/verify_microapp_time.XXXXXX.uapp)"
...
grep -a -q "time ok" "$RUN_LOG" || dump_log_and_fail "microapp run 路径未输出 time ok" "$RUN_LOG"
...
grep -a -q "time ok" "$LOADER_LOG" || dump_log_and_fail "loader-only 路径未输出 time ok" "$LOADER_LOG"
