...
RUN_LOG="$(mktemp /tmp/verify_microapp_bss_run.XXXXXX.log)"
LOADER_LOG="$(mktemp /tmp/verify_microapp_bss_loader.XXXXXX.log)"
BUILD_LOG="$(mktemp /tmp/verify_microapp_bss_build.XXXXXX.log)"
UAPP_PATH="$(mktemp /tmp/verify_microapp_bss.XXXXXX.uapp)"
...
grep -a -q "bss ok" "$RUN_LOG" || dump_log_and_fail "microapp run 路径未输出 bss ok" "$RUN_LOG"
...
grep -a -q "bss ok" "$LOADER_LOG" || dump_log_and_fail "loader-only 路径未输出 bss ok" "$LOADER_LOG"
