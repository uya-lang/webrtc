...
LEGACY_POBJ_V5="$(mktemp /tmp/verify_microapp_verify_v5.XXXXXX.pobj)"
LEGACY_POBJ_V6="$(mktemp /tmp/verify_microapp_verify_v6.XXXXXX.pobj)"
LEGACY_UAPP_V1="$(mktemp /tmp/verify_microapp_verify_v1.XXXXXX.uapp)"
POBJ_LOG="$(mktemp /tmp/verify_microapp_verify_pobj.XXXXXX.log)"
UAPP_LOG="$(mktemp /tmp/verify_microapp_verify_uapp.XXXXXX.log)"
POBJ_V5_LOG="$(mktemp /tmp/verify_microapp_verify_pobj_v5.XXXXXX.log)"
POBJ_V6_LOG="$(mktemp /tmp/verify_microapp_verify_pobj_v6.XXXXXX.log)"
UAPP_V1_LOG="$(mktemp /tmp/verify_microapp_verify_uapp_v1.XXXXXX.log)"
...
cleanup() {
    rm -f "$POBJ" "$UAPP" "$LEGACY_POBJ_V5" "$LEGACY_POBJ_V6" "$LEGACY_UAPP_V1" \
        "$POBJ_LOG" "$UAPP_LOG" "$POBJ_V5_LOG" "$POBJ_V6_LOG" "$UAPP_V1_LOG" "$BAD_LOG"
}
trap cleanup EXIT
...
relocs = payload[source_path_len + code_len + rodata_len + data_len:source_path_len + code_len + rodata_len + data_len + reloc_len]
