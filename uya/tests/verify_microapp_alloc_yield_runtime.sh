...
SOURCE="$ROOT_DIR/tests/fixtures/microapp/test_std_microapp_alloc_yield.uya"
RUN_LOG="$(mktemp /tmp/verify_microapp_alloc_yield_run.XXXXXX.log)"
LOADER_LOG="$(mktemp /tmp/verify_microapp_alloc_yield_loader.XXXXXX.log)"
BUILD_LOG="$(mktemp /tmp/verify_microapp_alloc_yield_build.XXXXXX.log)"
UAPP_PATH="$(mktemp /tmp/verify_microapp_alloc_yield.XXXXXX.uapp)"
...
grep -a -q "alloc yield ok" "$RUN_LOG" || dump_log_and_fail "microapp run 路径未输出 alloc yield ok" "$RUN_LOG"
...
grep -a -q "alloc yield ok" "$LOADER_LOG" || dump_log_and_fail "loader-only 路径未输出 alloc yield ok" "$LOADER_LOG"
...
echo "microapp alloc+yield runtime ok"
