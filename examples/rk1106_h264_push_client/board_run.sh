...
H264_GOP=${H264_GOP:-15}
FASTBOOT_VENC_CHANNEL=${FASTBOOT_VENC_CHANNEL:-0}
FASTBOOT_STARTUP_VENC_CHANNEL=${FASTBOOT_STARTUP_VENC_CHANNEL:-1}
...
FASTBOOT_STARTUP_VIDEO_WIDTH=${FASTBOOT_STARTUP_VIDEO_WIDTH:-$((FASTBOOT_VIDEO_WIDTH / 2))}
FASTBOOT_STARTUP_VIDEO_HEIGHT=${FASTBOOT_STARTUP_VIDEO_HEIGHT:-$((FASTBOOT_VIDEO_HEIGHT / 2))}
FASTBOOT_H264_BITRATE=${FASTBOOT_H264_BITRATE:-$H264_BITRATE}
FASTBOOT_H264_START_BITRATE=${FASTBOOT_H264_START_BITRATE:-$((FASTBOOT_H264_BITRATE / 4))}
FASTBOOT_H264_RAMP_FRAMES=${FASTBOOT_H264_RAMP_FRAMES:-$((FASTBOOT_VIDEO_FPS * 3))}
...
HELPER_STDOUT_LOG=${HELPER_STDOUT_LOG:-/dev/null}
HELPER_STDERR_LOG=${HELPER_STDERR_LOG:-/dev/null}
PRINT_LOGS_ON_SUCCESS=${PRINT_LOGS_ON_SUCCESS:-0}
...
if [ -z "$MEDIA_PATH" ]; then
    ...
    echo "board_run: fastboot_h264_gop=$FASTBOOT_H264_GOP" >&2
    echo "board_run: audio_disabled=$DISABLE_AUDIO" >&2
fi
...
if [ -z "$MEDIA_PATH" ]; then
    ...
    if [ "$DISABLE_AUDIO" = "1" ]; then
        FASTBOOT_H264_OUT="$FIFO_PATH" FASTBOOT_VENC_CHANNEL="$FASTBOOT_VENC_CHANNEL" FASTBOOT_STARTUP_VENC_CHANNEL="$FASTBOOT_STARTUP_VENC_CHANNEL" FASTBOOT_VIDEO_WIDTH="$FASTBOOT_VIDEO_WIDTH" FASTBOOT_VIDEO_HEIGHT="$FASTBOOT_VIDEO_HEIGHT" FASTBOOT_STARTUP_VIDEO_WIDTH="$FASTBOOT_STARTUP_VIDEO_WIDTH" FASTBOOT_STARTUP_VIDEO_HEIGHT="$FASTBOOT_STARTUP_VIDEO_HEIGHT" FASTBOOT_VIDEO_FPS="$FASTBOOT_VIDEO_FPS" FASTBOOT_H264_BITRATE="$FASTBOOT_H264_BITRATE" FASTBOOT_H264_START_BITRATE="$FASTBOOT_H264_START_BITRATE" FASTBOOT_H264_RAMP_FRAMES="$FASTBOOT_H264_RAMP_FRAMES" FASTBOOT_H264_GOP="$FASTBOOT_H264_GOP" "$DIR/fastboot_h264_fifo" >"$HELPER_STDOUT_LOG" 2>"$HELPER_STDERR_LOG" &
    else
        FASTBOOT_H264_OUT="$FIFO_PATH" FASTBOOT_VENC_CHANNEL="$FASTBOOT_VENC_CHANNEL" FASTBOOT_STARTUP_VENC_CHANNEL="$FASTBOOT_STARTUP_VENC_CHANNEL" FASTBOOT_AUDIO_OUT="/tmp/fastboot.g711" FASTBOOT_AUDIO_CARD="$FASTBOOT_AUDIO_CARD" FASTBOOT_VIDEO_WIDTH="$FASTBOOT_VIDEO_WIDTH" FASTBOOT_VIDEO_HEIGHT="$FASTBOOT_VIDEO_HEIGHT" FASTBOOT_STARTUP_VIDEO_WIDTH="$FASTBOOT_STARTUP_VIDEO_WIDTH" FASTBOOT_STARTUP_VIDEO_HEIGHT="$FASTBOOT_STARTUP_VIDEO_HEIGHT" FASTBOOT_VIDEO_FPS="$FASTBOOT_VIDEO_FPS" FASTBOOT_H264_BITRATE="$FASTBOOT_H264_BITRATE" FASTBOOT_H264_START_BITRATE="$FASTBOOT_H264_START_BITRATE" FASTBOOT_H264_RAMP_FRAMES="$FASTBOOT_H264_RAMP_FRAMES" FASTBOOT_H264_GOP="$FASTBOOT_H264_GOP" "$DIR/fastboot_h264_fifo" >"$HELPER_STDOUT_LOG" 2>"$HELPER_STDERR_LOG" &
    fi
    FASTBOOT_PID=$!
...
if [ "$DISABLE_AUDIO" = "1" ]; then
    echo "board_run: audio disabled" >&2
else
    echo "board_run: audio_fifo=/tmp/fastboot.g711" >&2
fi
set +e
export UYA_RK1106_DISABLE_AUDIO="$DISABLE_AUDIO"
if [ "$DISABLE_AUDIO" != "1" ]; then
