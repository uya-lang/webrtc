...
static int sns_width;
static int sns_height;
...
static int get_board_info(void)
{
	...
	if (metaVirmem != MAP_FAILED) {
		...
		sns_width = pSensorInitParam->cam_w;
		sns_height = pSensorInitParam->cam_h;
	} else {
		printf("mmap fail.\n");
		return -1;
	}
...
static void *GetMediaBuffer0(void *arg) {
	...
	while (!quit) {
		...
		if (s32Ret == RK_SUCCESS) {
			...
			if ((loopCount >= SAVE_ENC_FRM_CNT_MAX) && ((loopCount % 500)) == 0) {
				MPP_CHN_S stSrcChn, stDestChn;
				VENC_CHN_ATTR_S stAttr;
				VI_CHN_ATTR_S stChnAttr;

				RK_LOGI(">>>>>> Will do change resolutionn to: %d x %d", venc_w[index_w_h % index_num],
						venc_h[index_w_h % index_num]);
				...
				RK_LOGI("RK_MPI_SYS_UnBind Done");
				...
				RK_LOGI("RK_MPI_VENC_GetChnAttr Done");
				...
				RK_LOGI("RK_MPI_VENC_SetChnAttr Done");
				...
				RK_LOGI("RK_MPI_VI_GetChnAttr Done");
				...
				RK_LOGI("RK_MPI_VI_SetChnAttr Done");
				...
				RK_LOGI("RK_MPI_SYS_Bind Done, next will change to reso ind:%d", index_w_h + 1);
				index_w_h++;
			}
#endif
#if (ENABLE_RTSP)
...
static RK_S32 test_venc_init(int chnId, int width, int height, RK_CODEC_ID_E enType) {
	...
	if (enType == RK_VIDEO_ID_AVC) {
		...
		fprintf(stderr, "fastboot_client: H264 profile=baseline\n");
	} else if (enType == RK_VIDEO_ID_HEVC)
...
int vi_chn_init(int channelId, int width, int height) {
	...
	vi_chn_attr.stIspOpt.stMaxSize.u32Width = sns_width;
	vi_chn_attr.stIspOpt.stMaxSize.u32Height = sns_height;
